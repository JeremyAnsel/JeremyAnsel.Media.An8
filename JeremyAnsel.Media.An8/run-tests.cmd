...
packages\coverlet "JeremyAnsel.Media.An8.Tests\bin\%Configuration%\netcoreapp3.0\JeremyAnsel.Media.An8.Tests.dll" --target "dotnet" --targetargs "test -f netcoreapp3.0 --no-build" --output "bld\coverage\results-netcoreapp3.0.xml" --format opencover
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

packages\coverlet "JeremyAnsel.Media.An8.Tests\bin\%Configuration%\net452\JeremyAnsel.Media.An8.Tests.dll" --target "dotnet" --targetargs "test -f net452 --no-build" --output "bld\coverage\results-net452.xml" --format opencover
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
